...
export default function Quiz() {
  ...
  const current = quizSetOne[currentQuestion];
  ...
  const handleNext = () => {
    checkAnswer();
    setSelectedOptions([]);
    if (currentQuestion + 1 < quizSetOne.length) {
  ...
  const percentage = Math.round((score / quizSetOne.length) * 100);
  ...
  return (
      ...
      <div className="max-w-3xl mx-auto rounded-2xl p-8 shadow-lg space-y-6">
        <h1 className="text-4xl font-bold font-mono text-center text-emerald-400">Training Quiz 1</h1>

        {isFinished ? (
          <div className="text-center space-y-4">
            <p className="text-xl font-semibold">Final Score: {score} / {quizSetOne.length}</p>
          ...
          <>
            ...
            <div className="text-right">
              ...
              >
                {currentQuestion + 1 < quizSetOne.length ? "Next" : "Finish"}
              </button>
            </div>
          </>
        )}
